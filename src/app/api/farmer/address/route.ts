...
export async function GET(request: Request) {
  try {
    ...
    if (!mobileNo) {
      return NextResponse.json(
        { 
          ...
          message: 'Mobile number is required to fetch farmer address',
          data: null
        }, 
        { status: 400 }
      );
    ...
    const url = `${BZ_API}/Home/GetFarmerAddress?MobileNo=${cleanMobileNo}`;
    console.log('Fetching farmer address:', url);
